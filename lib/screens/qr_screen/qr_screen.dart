        ...
        children: [
          MobileScanner(
            ...
            onDetect: (capture) async{
              final List<Barcode> barcodes = capture.barcodes;
              final Uint8List? image = capture.image;
              int scannedBarcodes = 0;
              final totalBarcodes = barcodes.length;

              for (final barcode in barcodes) {
                debugPrint('Barcode found! ${barcode.rawValue.toString()}');
                if (barcode == barcodes.first) {
                  await con.handleBarcode(barcode.rawValue.toString());
                }
                await Future.delayed(Duration(seconds: 1)); // Add a delay of 1 second between each iteration
                scannedBarcodes++;
                setState(() {
                  _count = (scannedBarcodes.toDouble() / totalBarcodes.toDouble() * 100).toInt();
                });
